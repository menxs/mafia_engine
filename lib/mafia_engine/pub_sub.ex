defmodule MafiaEngine.PubSub do
  @moduledoc false
  def sub(game_id, ref), do: Registry.register(Registry.GamePubSub, game_id, ref)

  def sub_player(game_id, name, ref) do
    {:ok, _pid} = sub(game_id, name)
    unsub(game_id, ref)
  end

  def unsub(game_id, ref), do: Registry.unregister_match(Registry.GamePubSub, game_id, ref)

  def unsub_player(game_id, name), do: unsub(game_id, name)

  def unsub_all(game_id), do: Registry.unregister(Registry.GamePubSub, game_id)

  def pub(game_id, message),
    do:
      Registry.dispatch(Registry.GamePubSub, game_id, fn entries ->
        for {pid, _} <- entries, do: send(pid, message)
      end)

  def pub_player(game_id, name, message),
    do:
      Registry.dispatch(Registry.GamePubSub, game_id, fn entries ->
