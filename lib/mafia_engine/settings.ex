defmodule MafiaEngine.Settings do
  ...
  def change_role(s, inc_or_dec, "mafioso", n_players),
    do: change_role(s, inc_or_dec, :mafioso, n_players)

  def change_role(s, inc_or_dec, "sheriff", n_players),
    do: change_role(s, inc_or_dec, :sheriff, n_players)

  def change_role(s, inc_or_dec, "doctor", n_players),
    do: change_role(s, inc_or_dec, :doctor, n_players)
  ...
  def sum_roles(roles), do: Enum.reduce(roles, 0, fn {_, n}, acc -> n + acc end)
  ...
  def change_timer(s, :inc, phase) when is_atom(phase),
    do: %{s | timer: Map.update!(s.timer, phase, &inc_step/1)}

  def change_timer(s, :dec, phase) when is_atom(phase),
    do: %{s | timer: Map.update!(s.timer, phase, &dec_step/1)}
