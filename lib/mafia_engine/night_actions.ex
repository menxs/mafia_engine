defmodule MafiaEngine.NightActions do
  ...
  @spec select(t, MafiaEngine.Player.t(), MafiaEngine.Player.t()) ::
          {:ok, t}
          | {:error, reason :: atom}
  def select(night_actions, %Player{} = actor, %Player{} = target) do
    case check_action(actor, target) do
      :ok ->
  ...
  defp mafia_attacks(events) do
    case Enum.split_with(events, fn {_actor, role, _target} -> role == :mafioso end) do
      {[_ | _] = mafioso_events, events} ->
        mafia_target =
          mafioso_events
  ...
  defp doctor_heals(events) do
    case Enum.split_with(events, fn {_actor, role, _target} -> role == :doctor end) do
      {[_ | _] = doctor_events, events} ->
  ...
  defp process_doctor_action({_actor, :kill, target} = event, doctor_events) do
    healed =
      Enum.find(doctor_events, :none, fn {_actor, _role, heal_target} ->
        target == heal_target
      end)
  ...
  defp process_sheriff_action({actor, :sheriff, target}), do: {actor, :investigate, target}
  defp process_sheriff_action(event), do: event

  @spec check_action(MafiaEngine.Player.t(), MafiaEngine.Player.t()) ::
