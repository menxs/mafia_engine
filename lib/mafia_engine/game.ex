defmodule MafiaEngine.Game do
  ...
  def start_link(game_id),
    do: GenStateMachine.start_link(__MODULE__, game_id, name: via_tuple(game_id))

  def via_tuple(game_id), do: {:via, Registry, {Registry.Game, game_id}}

  # ----------------------------------------------------------------------------- Afterinterface

  def next_phase(game), do: GenStateMachine.cast(via_tuple(game), :next_phase)

  def started?(game), do: GenStateMachine.call(via_tuple(game), :started?)

  # ----------------------------------------------------------------------------- Afterinterface

  def add_player(game, name), do: GenStateMachine.call(via_tuple(game), {:add_player, name})

  def remove_player(game, name), do: GenStateMachine.cast(via_tuple(game), {:remove_player, name})

  def increase_role(game, role),
    do: GenStateMachine.cast(via_tuple(game), {:change_role, :inc, role})

  def decrease_role(game, role),
    do: GenStateMachine.cast(via_tuple(game), {:change_role, :dec, role})

  def increase_timer(game, phase),
    do: GenStateMachine.cast(via_tuple(game), {:change_timer, :inc, phase})

  def decrease_timer(game, phase),
    do: GenStateMachine.cast(via_tuple(game), {:change_timer, :dec, phase})

  def start_game(game), do: GenStateMachine.cast(via_tuple(game), :start_game)

  def talk_to(game, author), do: GenStateMachine.call(via_tuple(game), {:talk_to, author})

  def accuse(game, accuser, accused),
    do: GenStateMachine.cast(via_tuple(game), {:accuse, accuser, accused})

  def withdraw(game, accuser), do: GenStateMachine.cast(via_tuple(game), {:withdraw, accuser})

  def vote_innocent(game, voter),
    do: GenStateMachine.cast(via_tuple(game), {:vote, :innocent, voter})

  def vote_guilty(game, voter), do: GenStateMachine.cast(via_tuple(game), {:vote, :guilty, voter})

  def remove_vote(game, voter), do: GenStateMachine.cast(via_tuple(game), {:remove_vote, voter})

  def select(game, actor, target),
    do: GenStateMachine.cast(via_tuple(game), {:select, actor, target})

  def unselect(game, actor), do: GenStateMachine.cast(via_tuple(game), {:unselect, actor})
  ...
  @impl true
  def init(game_id) do
    {:ok, :initializing, fresh_data(game_id), [{:next_event, :internal, {:set_state, game_id}}]}
  end

  @impl true
  def terminate({:shutdown, _reason}, _state, _data) do
    :ok
  end
  ...
  def initialized({:call, from}, {:add_player, name}, data) do
    case Players.add(data.players, name) do
      {:ok, players} ->
        updated_data =
          data
        ...
        success(:keep_state, :initialized, updated_data, [{:reply, from, {:ok, players}}])

      {:error, reason} ->
        reply_error(from, {:error, reason})
    end
  end
  ...
  def initialized({:call, from}, :started?, _),
  ...
  def initialized(:cast, :start_game, data) do
    ...
    _xd = PubSub.pub_roles(data.game_id, players)
    PubSub.pub(data.game_id, {:game_update, :state, :playing})
    success(:next_state, :afternoon, updated_data)
  end

  def initialized({:timeout, :afk}, :exterminate, _data), do: {:stop, {:shutdown, :timeout}}
  ...
  def morning(:enter, _old_state, data),
    do:
  ...
  def morning({:call, from}, {:talk_to, name}, data) do
    case Players.get(data.players, name) do
      %Player{} = author ->
        {:keep_state_and_data, [{:reply, from, {author.alive, :everyone}}]}

      :none ->
        reply_error(from, {:error, :unknown_author})
    end
  end

  def morning(:cast, :next_phase, _data),
    do: {:keep_state_and_data, [{:state_timeout, 0, :go_next}]}

  def morning(:state_timeout, :go_next, data), do: success(:next_state, :accusation, data)

  def morning({:timeout, :afk}, :exterminate, _data), do: {:stop, {:shutdown, :timeout}}
  ...
  def accusation(:enter, :initializing, data),
    do:
  ...
  def accusation(:cast, {:accuse, accuser_name, accused_name}, data) do
    with %Player{} = accuser <- Players.get(data.players, accuser_name),
         ...
         true <- accused.alive,
         {:ok, accusations} <- Accusations.accuse(data.accusations, accuser, accused) do
  ...
  def accusation(:cast, :next_phase, _data),
    do: {:keep_state_and_data, [{:state_timeout, 0, :go_next}]}

  def accusation(:state_timeout, :go_next, data), do: success(:next_state, :afternoon, data)

  def accusation({:timeout, :afk}, :exterminate, _data), do: {:stop, {:shutdown, :timeout}}
  ...
  def defense(:cast, :next_phase, _data),
    do: {:keep_state_and_data, [{:state_timeout, 0, :go_next}]}

  def defense(:state_timeout, :go_next, data), do: success(:next_state, :judgement, data)

  def defense({:timeout, :afk}, :exterminate, _data), do: {:stop, {:shutdown, :timeout}}
  ...
  def judgement(:enter, _old_state, data),
    do:
  ...
  def judgement(:cast, {:vote, vote, voter_name}, data) do
    with %Player{} = voter <- Players.get(data.players, voter_name),
         true <- voter.alive,
         {:ok, votes} <- Votes.vote(data.votes, vote, voter) do
  ...
  def judgement(:cast, :next_phase, _data),
    do: {:keep_state_and_data, [{:state_timeout, 0, :go_next}]}
    ...
    updated_data =
      case Votes.result(data.votes) do
        :guilty ->
          ...
          PubSub.pub(
            data.game_id,
            {:game_update, :role, {accused, Players.get(data.players, accused).role}}
          )
  ...
  def judgement({:timeout, :afk}, :exterminate, _data), do: {:stop, {:shutdown, :timeout}}
  ...
  def afternoon(:enter, _old_state, data),
    do:
  ...
  def afternoon(:cast, :next_phase, _data),
    do: {:keep_state_and_data, [{:state_timeout, 0, :go_next}]}

  def afternoon(:state_timeout, :go_next, data), do: success(:next_state, :night, data)

  def afternoon({:timeout, :afk}, :exterminate, _data), do: {:stop, {:shutdown, :timeout}}
  ...
  def night(:enter, :initializing, data),
    do:
  ...
  def night(:cast, {:select, actor_name, target_name}, data) do
    with %Player{} = actor <- Players.get(data.players, actor_name),
         ...
         {:ok, night_actions} <- NightActions.select(data.night_actions, actor, target) do
  ...
  def night({:call, from}, {:talk_to, name}, data) do
    case Players.get(data.players, name) do
      %Player{} = author ->
        {:keep_state_and_data,
         [{:reply, from, {author.alive and author.role == :mafioso, :mafioso}}]}
  ...
  def night(:cast, :next_phase, _data),
    do: {:keep_state_and_data, [{:state_timeout, 0, :go_next}]}
  ...
  def night({:timeout, :afk}, :exterminate, _data), do: {:stop, {:shutdown, :timeout}}
  ...
  def game_over({:call, from}, {:talk_to, _name}, _data),
    do: {:keep_state_and_data, [{:reply, from, {true, :everyone}}]}

  def game_over(:cast, :next_phase, _data),
    do: {:keep_state_and_data, [{:state_timeout, 0, :exterminate}]}

  def game_over(:state_timeout, :exterminate, _data), do: {:stop, {:shutdown, :game_ended}}
  ...
  defp update_votes(data, votes), do: %{data | votes: votes}

  defp update_night_actions(data, night_actions), do: %{data | night_actions: night_actions}
  ...
  defp process_events(players, events, game_id) do
    process_fun = fn
      {_actor, :kill, target}, acc_players ->
        PubSub.pub(
          game_id,
          {:game_update, :role, {target, Players.get(acc_players, target).role}}
        )
  ...
  defp reply_error(from, response), do: {:keep_state_and_data, [{:reply, from, response}]}
end
